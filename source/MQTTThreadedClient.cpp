...
MQTTThreadedClient::MQTTThreadedClient(char* deviceId, NetworkInterface * aNetwork)
        : network(aNetwork),
          port(8883),
          ...
          useTLS(true)
{
        ...
        ssl_ca = MBED_CLOUD_DEV_LWM2M_SERVER_ROOT_CA_CERTIFICATE;
        ssl_ca_len = sizeof(MBED_CLOUD_DEV_LWM2M_SERVER_ROOT_CA_CERTIFICATE);

        ssl_client_cert = MBED_CLOUD_DEV_BOOTSTRAP_DEVICE_CERTIFICATE;
        ssl_client_cert_len = sizeof(MBED_CLOUD_DEV_BOOTSTRAP_DEVICE_CERTIFICATE);

        ssl_client_pkey = MBED_CLOUD_DEV_BOOTSTRAP_DEVICE_PRIVATE_KEY;
        ssl_client_pkey_len = sizeof(MBED_CLOUD_DEV_BOOTSTRAP_DEVICE_PRIVATE_KEY);
        ...
        setConnectionParameters(hostname, port, logindata);
        addTopicHandler(topic_1, messageArrived);
}
...
void MQTTThreadedClient::addTopicHandler(const char * topicstr, void (*function)(MessageData &))
{
    ...
    FPtr<void,MessageData &> fp;
    fp.attach(function);
    
    topicCBMap.insert(std::pair<std::string, FPtr<void,MessageData &> >(std::string(topicstr),fp));    
} 
