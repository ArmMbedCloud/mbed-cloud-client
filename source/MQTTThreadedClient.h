...
static const char* hostname = "ingest.mqtt.data.mbedcloudintegration.net";
...
{
public:
    MQTTThreadedClient(char* deviceId, NetworkInterface * aNetwork);
...
private:
    ...
    const uint8_t* ssl_ca;
    const uint8_t* ssl_client_cert;
    const uint8_t* ssl_client_pkey;
    ...
    std::map<std::string, FPtr<void, MessageData &> > topicCBMap;
